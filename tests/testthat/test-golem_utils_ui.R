...
test_that("Test undisplay works", {
  ...
  # Update expectations to include the span wrapper
  expected_html <- '<button id="go_filter" type="button" class="btn btn-default action-button">
  <span class="action-label">go</span>
</button>'

  expect_equal(as.character(b), expected_html)

  # For undisplay version
  expected_undisplay_html <- '<button id="go_filter" type="button" class="btn btn-default action-button" style="display: none;">
  <span class="action-label">go</span>
</button>'

  expect_equal(as.character(b_undisplay), expected_undisplay_html)
})
